...
- [ ] 3.1 — Build /dashboard layout (sidebar, header, wrapper, dark mode toggle)
- [ ] 3.2 — Build /dashboard page — Home (welcome banner, KPI tiles, quick actions, recent conversations)
- [ ] 3.3 — Build /dashboard/bots — My chatbots list (status toggle, create new bot button)
- [ ] 3.4 — Build /dashboard/bots/new — Bot setup wizard Step 1 (basic info: name, business type, personality)
- [ ] 3.5 — Build /dashboard/bots/new — Bot setup wizard Step 2 (knowledge base: paste FAQ or upload PDF)
- [ ] 3.6 — Build /dashboard/bots/new — Bot setup wizard Step 3 (appearance: color, position, welcome message, avatar)
- [ ] 3.7 — Build /dashboard/bots/[id] — Bot detail & settings (tabs: General, Knowledge Base, Appearance, Domains)
- [ ] 3.8 — Build /dashboard/bots/[id]/embed — Embed code page (script tag, copy button, install instructions, test widget)
- [ ] 3.9 — Build /dashboard/conversations — Conversations table with slide-over panel and filters
- [ ] 3.10 — Build /dashboard/analytics — Analytics page (date range picker, KPI row, 3 charts)
- [ ] 3.11 — Build /dashboard/settings — Account settings (business info, password change, team members, notifications)
- [ ] 3.12 — Build /dashboard/billing — Billing page (plan card, usage meter, upgrade buttons, billing history, Stripe portal link)
...
- [ ] 5.1 — Scaffold widget/botflow-widget.js — standalone vanilla JS file structure
- [ ] 5.2 — Implement widget launcher button (pill-shaped, avatar, "Chat with us" text)
- [ ] 5.3 — Implement chat window (320×480, open/close animation, shadow DOM or scoped CSS)
- [ ] 5.4 — Implement lead capture form (name + email, shown before first message)
- [ ] 5.5 — Implement message rendering (user right-aligned indigo, bot left-aligned gray, timestamps)
- [ ] 5.6 — Implement SSE streaming response rendering (tokens appear one by one)
- [ ] 5.7 — Implement typing indicator (3 animated dots)
- [ ] 5.8 — Implement conversation persistence via localStorage
- [ ] 5.9 — Implement mobile responsive behaviour (full screen on mobile)
- [ ] 5.10 — Implement widget initialization flow (async load → fetch config → inject DOM → handle messages)

---

## PHASE 6: BILLING (STRIPE)

- [ ] 6.1 — Define PLAN_LIMITS constants (Starter / Growth / Pro — chats, bots, team members)
- [ ] 6.2 — Integrate Stripe Checkout for new subscriptions
- [ ] 6.3 — Integrate Stripe Customer Portal for plan changes and payment methods
- [ ] 6.4 — Handle webhook events: checkout.session.completed, subscription.updated, subscription.deleted
- [ ] 6.5 — Enforce monthly chat limits — return 429 with upgrade prompt when exceeded

---

## PHASE 7: LANDING PAGE

- [ ] 7.1 — Build Hero section (headline, subheadline, CTA buttons, live demo widget)
- [ ] 7.2 — Build Social proof section ("Trusted by X businesses" logos/count)
- [ ] 7.3 — Build How it works section (3 steps)
- [ ] 7.4 — Build Features section (6 feature cards with icons)
- [ ] 7.5 — Build Industry use cases section (tabs: Salon / Restaurant / Dental / Auto Dealership)
- [ ] 7.6 — Build Pricing section (3 plan cards with feature comparison table)
- [ ] 7.7 — Build Testimonials section (3 quote cards)
- [ ] 7.8 — Build CTA section (email input + "Start free trial" button)
- [ ] 7.9 — Build Footer (links, social, legal)
